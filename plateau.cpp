...
void Plateau::removePionTest()
{
    QList<Pion* > a;
    a.clear();

    a.append(pionAt(QPair<int,int >(3,5)));
     a.append(pionAt(QPair<int,int >(5,3)));
     a.append(pionAt(QPair<int,int >(4,7)));
a.append(pionAt(QPair<int,int >(4,3)));
...
void Plateau::testConc()
{
 ...
 void Plateau::uptadeMove()
 {
     ...
     {//_p1 empty 2 has won
         _winner=2;
         _toolTip->setHtml("Le joueur n°2 a gagné");
     }
      if(!_p2.isEmpty())
     {
      ...
      {
         _winner=1;
         _toolTip->setHtml("Le joueur n°1 a gagné");
     }
 ...
 void Plateau::uptadeMoveHelper(Pion* p)
 {
     if(!canPionMove(p))
     {
      ...
      if(!_reachedToBeDeleted.isEmpty())
         {
         ...
         if(_reachedToBeDeleted.isEmpty() && _leavedToBeDeleted.isEmpty())
         {
             p->setColor(QColor());
         }
...
//move is already possible and pion on position "cr"
...
//i is used here don't delete!
i=0;

    while(k.first>=0 && k.second>=0 && k.first<7 && k.second<9 && i<70)
    {
              k.first=k.first+dX;
              k.second= k.second + dY;
 ...
 bool Plateau::isMovePossible(QPair <int,int> prev,QPair <int,int> c,Pion* p)
 {
     ...
     {//position occupé par un pion déjà fait en début de meth appelante
            if(!_prevC.isEmpty())
            {
                ...
                {
                    _toolTip->setHtml("<p>Vous êtes déjà passé par ce point ou c'est un déplacement interdit</p>");
         ...
         if(_lastDeleted)
         {
              QPair <int,int> ld= _lastDeleted->getCord();
              int dx= c.first-prev.first;
              int dy= c.second-prev.second;


              if(prev.first==(ld.first+2*dx) && prev.second==(ld.second+2*dy))
                  {
                  //_prevC.append(c);
                  return false;
              }
...
if(_mode!=player1CanDelete && _mode!=player2CanDelete)
...

    {
        if(!isTurnToMove(p))
        {
            _toolTip->setHtml("<i>Ce n'est plus votre tour</i>");
        }
...
if(c != -1)
{
    ...
    //permet d'éliminer les situations blocantes
    if(_playerMustStrike)
    {
  QList< QPair <int,int> > adj= adjacentCoord(prevC);
  //qDebug()<<prevC;
  int u=0;
            for(int i=0;i<adj.length();i++)
            {
              QPair <int,int> a= adj.at(i);
               if(isMovePossible(prevC,a,p))
                  u++;
            }
    ...
    //qDebug()<<"pass = "<<pass;
            ...
            if( _playerMustStrike && playerMustStrike(p) && !pass)
            {
...
//cette partie n'est peut etre pas efficace et peut être inutile ('posbMov')

 QList< QPair <int,int> > adj= adjacentCoord(p->getCord());
 QList< QPair <int,int> > possibleMove;
 possibleMove.clear();
 ...
 for(int i=0;i<adj.length();i++)
 {
     ...
     if(!_prevC.isEmpty())
     {
         if(!_prevC.contains(a) && pionAt(a)==0)
             possibleMove.append(a);
     }
 }

 posbMov=possibleMove.length();

        if(_p1ThatCanStrike.contains(p) && posbMov!=1 && posbMov!=0)
            return true;
    }
    ...
    if(p->getOwner()==2)
    {
        ...
        if(_p2ThatCanStrike.contains(p) && posbMov!=1 && posbMov!=0)
            return true;
    }
...
void Plateau::initMap()
{
    ...
    for(int i=0;i<9;i++)
    {
        ...
        for(int j=0;j<7;j++)
        {
             ...
             //g(j,i) j: ligne , i colonne, le premier est représente toujours une ligne la numérotation commence à zéro
            QPair<int,int> g(j,i);
            _c.append(g);
...
if(p){
         if(_leavedToBeDeleted.contains(p))
         {
             choix=2;
              ...
              _lastDeleted= _leavedToBeDeleted.first();
